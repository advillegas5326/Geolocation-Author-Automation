...
def create_execution_variables(database, table):
  ...
  complete_countries = (initial_db,country_prediction,input_language,table_results, final_table_path, cities_table, path_to_save, save_name)
...
def complement_or_complete_data(country,splits):
  ...
  if not DeltaTable.isDeltaTable(spark, f'/user/hive/warehouse/{temp_table}.db/{country[3]}'):
    non_analyzed_dataframe = spark.sql("select * from {}".format(country[0])).toPandas()
    ...
    file_path = dataframe_to_csv(non_analyzed_dataframe,country[6], country[7])
    print("Path: ",file_path)
    size = non_analyzed_dataframe.shape[0] // int(splits)
    chunk_paths = csv_splitter(size, country, non_analyzed_dataframe, splits)
    frame = pd.DataFrame(chunk_paths)
    display(frame)
    return chunk_paths
  else:
    #MATCH TABLE WITH SAVED TABLE (HIDRATED AND GEOLOCATED)
    non_analyzed_dataframe = spark.sql(f"SELECT {country[0]}.* FROM {country[0]} LEFT JOIN {country[4]} ON ({country[0]}.SN_MSG_ID = {country[4]}.SN_MSG_ID) WHERE {country[4]}.SN_MSG_ID IS NULL").toPandas()
    print("Old size: ",non_analyzed_dataframe.shape)
    non_analyzed_dataframe = non_analyzed_dataframe.drop_duplicates(subset='SN_MSG_ID', keep="first")
...
def csv_splitter(size, country, frame, splits):
  ...
  counter = 0
  save_paths = []
  list_of_dfs = [frame.loc[i:i+size-1,:] for i in range(0, len(frame),size)]
